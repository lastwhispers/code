 ...
 *
 *  2）AnnotationAwareAspectJAutoProxyCreator（这个组件什么时候工作）
 ...
 *
 *      在整个继承结构中，给关注点设置断点
 *      AbstractAutoProxyCreator.setBeanFactory();因为它获取了BeanFactory
 *      AbstractAutoProxyCreator.postProcessAfterInitialization、postProcessBeforeInstantiation；因为它有后置处理器的逻辑
 *
 *      AbstractAdvisorAutoProxyCreator.setBeanFactory();重写了setBeanFactory()，执行自己的initBeanFactory()
 *
 *      AspectJAwareAdvisorAutoProxyCreator没有需要关注的点
 *
 *      AnnotationAwareAspectJAutoProxyCreator.initBeanFactory；重新父类initBeanFactory()方法
 *
 ...
 *      PostProcessorRegistrationDelegate.registerBeanPostProcessors()：注册bean的后置处理器，用于拦截bean的创建
 *         *注册bean的后置处理器的流程：
 ...
 *              *AnnotationAwareAspectJAutoProxyCreator注册流程（225行：BeanPostProcessor pp = beanFactory.getBean(ppName, BeanPostProcessor.class);）
 *                  1.创建名称为internalAutoProxyCreator的BeanPostProcessor（AnnotationAwareAspectJAutoProxyCreator）实例
 *                  504行：doCreateBean()
 *                      internalAutoProxyCreator——》513行：instanceWrapper = createBeanInstance(beanName, mbd, args)
 *                      AnnotationAwareAspectJAutoProxyCreator——》515行：final Object bean = instanceWrapper.getWrappedInstance()
 ...
 *   =======以上是注册AnnotationAwareAspectJAutoProxyCreator的过程========
 *
 ...
 *                  1）、将目标方法的增强器（Advice），包装成截器链（MethodInterceptor）
